...
class GQADataset:
    ...
    def __init__(self, splits: str):
        ...
        for split in self.splits:
            self.data.extend(
                # DGX= "/proj/"
                # json.load(open("/users/avmendoz/data/gqa/%s.json" % split))
                json.load(open("/users/avmendoz/data/gqa/%s.json" % split))
            )
...
class GQABufferLoader:
    ...
    def load_data(self, name, number):
        if name == "testdev":
            path = "/users/avmendoz/data/gqa/data/vg_gqa_imgfeat/gqa_testdev_obj36.tsv"
        else:
            path = "/users/avmendoz/data/gqa/data/vg_gqa_imgfeat/vg_gqa_obj36.tsv"
...
class GQATorchDataset(Dataset):
    def __init__(self, dataset: GQADataset):
        ...
        img_data = []
        if (
            "testdev" in dataset.splits or "testdev_all" in dataset.splits
