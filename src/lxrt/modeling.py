...
class BertConfig(object):
    ...
    def __init__(
        ...
        """
        vocab_size_or_config_json_file = BERT_DIR + "bert_config.json"
...
class BertPreTrainedModel(nn.Module):
    ...
    def from_pretrained(
        ...
        **kwargs
        ...
        """
        pretrained_model_name_or_path = BERT_DIR
...
class LXRTPretraining(BertPreTrainedModel):
    def __init__(
        ...
        if self.task_cls:
            self.binary_head = BertClassificationHead(config)
    ...
    def forward(
        ...
        if self.task_cls:
            binClsFct = CrossEntropyLoss(ignore_index=-1)
            binary_logits = self.binary_head(pooled_output)
            binaryLoss = binClsFct(
                binary_logits.view(-1, 2), torch.max(union_label, 1)[1].view(-1)
            )
            total_loss += binaryLoss
            losses += (binaryLoss.detach(),)
        ...
        if ans is not None and self.task_qa:
            answer_loss = loss_fct(
                answer_score.view(-1, self.num_answers), ans.view(-1)
            )
